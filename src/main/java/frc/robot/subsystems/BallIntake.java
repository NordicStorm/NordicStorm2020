...
public class BallIntake extends Subsystem {
    ...
    int moveTime = 1000; // TODO find this
    int sensorMoveTime=500;

    int shooterFeedTime = 1000;
    ...
    public BallIntake() {
        ...
        stageOne = new WPI_TalonSRX(4);

        stageTwo = new WPI_TalonSRX(5);

        stageThree = new WPI_TalonSRX(6);

        stageFour = new WPI_TalonSRX(7);

        stageFive = new WPI_TalonSRX(8);
        ...
        for (int i = stageMotors.length - 2; i < 0; i--) {// -2 because top stage never moves (except for shooting)
            ...
            if (timeLeftToMoveFrom > 0 || timeLeftToMoveTo > 0) {
                ...
                if(timeLeftToMoveTo<=sensorMoveTime){
                    if(isOccupied || timeLeftToMoveTo<0){ // if the ball has gotten here, tell the previous stage and this one to stop
    ...
    public void setRawMotors(double a, double b, double c, double d, double e){
        stageMotors[0].set(a);
        stageMotors[1].set(b);
        stageMotors[2].set(c);
        stageMotors[3].set(d);
    ...
    /**
     * 
     * @return True if the ball was sucessfully fed up, false if it was not due to lack of ball or already-running feeder
     */
