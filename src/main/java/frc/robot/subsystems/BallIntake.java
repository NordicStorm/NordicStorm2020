...
private WPI_TalonSRX kicker;
    ...
    long timeKickerMoving=0;// approximation for current kicker speed
...
kicker = new WPI_TalonSRX(11);
    ...
    private void runBallsThroughSystem() {
        ...
        if (timeToStopKicker > now) {
            kicker.set(1);
            timeKickerMoving+=timePassed;
        } else {
            if(timeKickerMoving>0){
                timeKickerMoving-=timePassed;
                if(timeKickerMoving<0){
                    timeKickerMoving=0;
                }
        ...
        for (int i = stageMotors.length - 1; i >= 0; i--) {
            ...
            if(i==4 && timeKickerMoving<1000){
                kickerReady=false;
            }
            if ((timeStopFrom > now || timeStopTo > now ) && kickerReady) {
                runStageMotor = true;
    ...
    public boolean areBallsPresent(){
        return (stageSensors[0].get() || stageSensors[1].get()|| stageSensors[2].get() || stageSensors[3].get() || stageSensors[4].get())||
            (timeToStopStageBeingMovedTo[1]>now || timeToStopStageBeingMovedTo[2]>now || timeToStopStageBeingMovedTo[3]>now || timeToStopStageBeingMovedTo[4]>now) || //note, the first stage is omitted. This is because the intake may be spinning without balls
            (timeToStopStageBeingMovedFrom[1]>now || timeToStopStageBeingMovedFrom[2]>now || timeToStopStageBeingMovedFrom[3]>now || timeToStopStageBeingMovedFrom[4]>now || timeToStopKicker>now);
    }
