...
private WPI_TalonSRX kicker;
    ...
    WPI_TalonSRX[] stageMotors = new WPI_TalonSRX[4];
    DigitalInput[] stageSensors = new DigitalInput[4];
    long[] timeToStopStageBeingMovedTo = new long[] { 0, 0, 0, 0 };
    long[] timeToStopStageBeingMovedFrom = new long[] { 0, 0, 0, 0 };

    long timeToStopKicker = 0;
    double moveSpeed = 1;
    int moveTime = 500;
    int sensorMoveTime = 300;
    int timeToKick = 500;

    int shooterFeedTime = 600;
    int timeToIntake = 600;
...
kicker = new WPI_TalonSRX(11);
    ...
    private void runBallsThroughSystem() {
        boolean[] stageCanBeMovedInto = new boolean[] { false, false, false, false };
        ...
        for (int i = stageMotors.length - 1; i >= 0; i--) {
            ...
            if (timeStopFrom > now || timeStopTo > now) {
                ...
                if (timeLeftToMoveTo <= sensorMoveTime || i == 3) {
                ...
                if (isOccupied) {
                    ...
                    if (i != 3 && stageCanBeMovedInto[i + 1]) {
                        isAvailable = true;
                        ...
                        if (timeToStopStageBeingMovedFrom[i] < now) {
                            ...
                            timeToStopStageBeingMovedTo[i + 1] = now + moveTime + add;
                            runStageMotor = true;
    ...
    public boolean feedBallToShooter() {
        ...
        if (isBallAvailableToShoot()) {
            ...
            timeToStopStageBeingMovedFrom[3] = now + shooterFeedTime;
            return true;
        }
        return false;
    }
    ...
    public boolean isBallAvailableToShoot() {
        long now = System.currentTimeMillis();
        if (!stageSensors[3].get()) {
            return false;
        }

        boolean lastStageRunning = timeToStopStageBeingMovedFrom[3] > now || timeToStopStageBeingMovedTo[3] > now;
